---
title: "Detecting shape shifting across enviornmental gradients -- SMA residuals vs including mass as a covariate"
...
Due to its important implications for wildlife responses to climate change, studies of Allen's rule (i.e., that animals have relatively longer appendages in warmer climates to facilitate thermoregulation) have increased in recent decades. While there is general agreement that warm-blooded animals become longer in warmer climates, the methods used to quantify changes in relative appendage length have varied widely. Authors have taken four main approaches: (1) modeling simple ratios (e.g., length:mass) as a function of temperature, (2) modeling appendage length as a function of mass (to control for body size) and temperature, (3) conducting a two-step regression by (3a) modeling appendage length as a function of mass using ordinary least squares (OLS) regression, and then (3b) modeling the residuals as a function of temperature, and (4) the same as approach 3, but using standardized major axis (SMA) regression in step 3a.

Here, I compare these four approaches using simulated morphological data of 180 hypothetical species that vary in their response to increasing temperatures, and in the strength of appendage length–mass covariation. I find that ratio approaches are positively biased, while 'mass as a covariate' and two-step OLS approaches are negatively biased. Only the two-step approach using SMA regression (a type of type II regression) returns unbiased estimates of temperature's effect on relative appendage length. We find qualitatively similar results in a case study examining 'wingyness' across latitudinal gradients in three nightjar species from North America and Europe.

Despite being recommended for studies of allometric scaling, SMA regression has been rarely used in animal shape-shifting research. We recommend that the 'SMA residuals' approach (i.e., approach 4) become the standard method for assessing shape-shifting in wildlife.
...
1)  Absolute appendage length: Does not account for body size, e.g., \
    Appendage \~ Temperature_increase

2)  Ratio approach: Use the ratio of appendage length to mass as the response variable, e.g.,\
    Wing:mass \~ Temperature_increase

3)  'OLS residuals' approach: Using the residuals from a OLS regression model: E.g.\
    Mod 1: OLS(Wing \~ Mass) \# Extract the residuals from this model, THEN\
    Mod 2: OLS(Ols_residuals \~ Temperature_increase)

4)  'Body size as a covariate' approach: Include a metric of body size as a covariate in models predicting appendage length: E.g.,\
    OLS(Wing \~ Mass + Temperature_increase)

Thus the principles of biological scaling theory are well-suited to answering questions regarding spatial and temporal Allen's rule. With the goal of understanding the impact of temperature increase on changes in ***body shape*** (i.e., relative appendage length),

1)  'SMA residuals' approach: Using the residuals from a SMA regression model: E.g.\
    Mod 1: SMA(Wing \~ Mass) \# Extract the residuals from this model, THEN\
    Mod 2: OLS(Sma_residuals \~ Temperature_increase)

Approaches 2-4 attempt to control for body size (the variable 'Mass' in the above models) to isolate the changes in *relative* appendage length (variable 'Wing'), but approaches 2 and 3 are known to regularly produce biases. The ratio approach has been used for several decades, but has been shown to be biased as it is often highly correlated with body size [@greenMassLengthResiduals2001; @jakobEstimatingFitnessComparison1996]. Past simulation studies have compared multivariate regression (i.e., the 'Body size as a covariate' approach) and the OLS residuals approach, finding that multivariate regression does a better job of estimating the slope of the variable of interest (in this case, temperature increase), especially when predictor variables are correlated [@freckletonMisuseResidualsEcology2002].

While there are challenges with using residuals However, other studies show that OLS regression is not the appropriate type of regression for studies of allometric scaling, and that using the residuals from model II regression is more closely related to biological metrics of interest [e.g., body condition, @greenMassLengthResiduals2001; @peigNewPerspectivesEstimating2009; @peigParadigmBodyCondition2010]. In the context of detecting relative changes in appendage length, there are authors who have advocated for the the 'Mass as a covariate' approach [@rydingResponseAllometryEvaluate2022], while others have stressed the importance of using allometric scaling theory [@santoroAllometryEvaluateAllens2022]. To date, there is no consensus on which approach most accurately estimates the impact of an increase of temperature on body shape (i.e., relative appendage length).

Despite these known biases, ratios are still used to examine changes in animal shape [e.g., @jirinecMorphologicalConsequencesClimate2021; @dubinerWidespreadRecentChanges2022], thus, we include the ratio approach in the present study to facilitate comparison, but we do not discuss it further.
...
Lr %>% mutate(num_studies = sum(value == "P"), .by = approach) %>%
...
Peig & Green [-@peigNewPerspectivesEstimating2009] revolutionized the way we estimate body condition, using allometric scaling theory to create the standardized mass index (see 3 step process outlined on pages 1886 & 1887). We will try to employ a similar 3 step process, but instead create the standardized wing index (SWI).
...
As suggested by previous authors, SMA regression can improve our biological inference in studies involving biological scaling. SMA assumes that the error variance in X and Y are equivalent, which makes more biological sense than assuming all error is in the Y variable. However, this assumption may not always be reasonable, so another option is to directly model X and some error $\sigma_x$ in X. We model the true, unobserved value of the predictor ($X_{true}$) as being normally distributed around the observed value ($X_{obs}$) with some standard deviation $\sigma_x$, which represents the measurement error in X. Then we model the response variable as depending linearly on the latent $X_{true}$, with its own residual error $\sigma_y$.

$$
X_{true} ~ N(X_{obs}; \sigma_x)
$$

$$
Y ~ N(\alpha + \beta_1 * X_{true};  \sigma_y)
$$

This can be done in a Bayesian or maximum likelihood framework, and these models can be directly extended to include additional biological complexity (e.g., phylogenetic effects). The gold standard is to collect repeat measurements on morphological traits X and Y, and to model $\sigma_x$ and $\sigma_y$ explicitly. However, repeat measurements are rarely collected in ecology, and without them $\sigma_x$ and $\beta_x$ are not separately identifiable. Additional options that will more closely resemble biological reality (compared to OLS regression) are to:
...
Biological complexity, accounting for
