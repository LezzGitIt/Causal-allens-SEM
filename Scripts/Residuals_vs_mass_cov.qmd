---
title: "Detecting shape shifting across environmental gradients -- methods informed by allometric scaling have superior performance"
...
I simulate data for three variables: appendage length, body size, and temperature increase (Temperature), using a multivariate normal (MVN) distribution:
...
The equations in @sec-equations will make more sense as we code some examples. Remember, the correlation structure is specified in the order `(appendage, body size, temperature)`, such that, for example, ρ₁₂ is the correlation between appendage and body size. \
...
To illustrate the key properties of SMA regression, let's generate some example data & run some SMA models. We generate an isometric (b_sma = 0.33) species that becomes relatively 'lankier' with an increase in temperature, i.e. the species decreases more in body size (r_23 = -0.4) relative to appendage length (r_13 = -0.1) for a given increase in temperature.
...
Another property of SMA regression is that the residuals are equal & oppositely correlated with both $X$ (body size) & $Y$ (appendage). In OLS -- the residuals are not correlated with $X$ & highly correlated with $Y$.
...
The allometric scaling slope determines the rate of increase of the $Y$ variable (wing in this case) with an increase in the $X$ variable (mass). The intercept reflects the proportions of the appendage relative to body size such that when slopes are constant, the group with the higher intercept has proportionally larger wings across all body sizes [@shingletonAllometryStudyBiological2010].
...
We simulate data for `r nrow(parm_tbl)` hypothetical bird species (i.e., every combination of parameters in our parameter matrix) and test the assumptions of our models for each hypothetical bird species.
...
My interpretation of qq-plots & residual vs fitted plots suggest that the assumptions of SMA regression (table , Warton) are mostly being met. Do you agree with that? The results do change significantly if I use MA regression.
...
We see that the $\lambda$ values between `r min(lambdas)` - `r max(lambdas)` with a mean of `mean(lambdas)`. I interpret this as suggesting that the variance in wing is very low compared to the variance in mass (relative to the mean values), which may very well be true based on other studies examining precision of wing and mass in birds NEED MASS REF [@subasingheRepeatabilityValidityPhenotypic2021]. In this case, do you think it’s still appropriate to use SMA regression (note the $\lambda$ values from the empirical case study, @sec-case-study, are similar)? 
...
The difference in the 5 approaches (note that Wing / mass and Wing² / mass are both 'ratio approaches') is how wingyness is measured (as outlined in @sec-Conceptual-background). Note that the 'body size as a covariate' approach would also have $\beta_2 * \mathit{Body\ size}$.
...
# Function to pull the desired proportion & convert to a percent using the E
...
sali_proportional <- Proportional_methods_eval %>% 
  filter(Model == "SALI" & Direction == "pos") %>% 
  pull(Proportion)
# Difference between SALI classification & reality 
Dif_sali <- round(sali_proportional - Prop_wingyer, 2) * 100
```
