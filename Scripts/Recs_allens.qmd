---
title: "The Shape of Things to Come: A Perspective on Organismal Form Across Environmental Gradients"
...
Allometry is the study of how biological processes change with body size. In the context of relative appendage length, we can think of this as how an appendage $Y$ scales with body size $X$. Appendage length and body size are related through:
...
There has been confusion in the literature as to what constitutes adequately addressing allometric scaling in studies of body shape. In the context of examining relative appendage length, @santoroAllometryEvaluateAllens2022 argue that we must base our methods in allometric scaling theory, and imply that only certain types of regression (e.g., SMA regression) appropriately address allometric scaling. In a rebuttal by @rydingResponseAllometryEvaluate2022, they argue that the 'standard practice approach' of including body size as a covariate in multiple regression is 'effectively equivalent' to more 'detailed allometric investigations.' I agree with both sets of authors in that 1) any approach examining appendage length relative to body size does (at least somewhat) address allometric scaling [as implied by @rydingResponseAllometryEvaluate2022], but 2) methods that account for measurement error in $X$ are more robust (in most situations) than methods that ignore this measurement error [@santoroAllometryEvaluateAllens2022].
:::
...
## The hierarchical measurement-error model

I argue that both classical regression approaches and the SMA approach make assumptions that can be limiting, and that a hierarchical measurement-error approach provides a more flexible alternative that directly models measurement error in $X$ and $Y$ [@subasingheRepeatabilityValidityPhenotypic2021, Carriquiry, 2015, Ponzi et al 2018]. With repeat measurements $j$ of the morphological trait $X$ from individual $i$ we can obtain an estimate of measurement error ($\sigma_{x,within}$) of $X$. Simple algebra allows us to obtain an estimate of the between-individual variance ($\sigma_{x,between}$).

I argue that both classical regression approaches and the SMA approach make assumptions that can be limiting, and that a hierarchical measurement-error framework offers a more flexible and biologically realistic alternative by explicitly modeling measurement error in our predictor ($X$) [@subasingheRepeatabilityValidityPhenotypic2021, Carriquiry, 2015, Ponzi et al 2018]. Suppose we have repeat measurements $j$ of a morphological trait $X$ for individual $i$, denoted $X_{ij}$. These can be used to estimate the within-individual measurement error, $\sigma_{x,\text{within}}$, and — by comparing to the total observed variance — to infer the between-individual variance, $\sigma_{x,\text{between}}$.
